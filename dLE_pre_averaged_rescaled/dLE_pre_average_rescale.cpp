...
for (int i = 1; i < argc; ++i) {
  if(std::string(argv[i]) == "-h"){
    cout << " " <<endl;
    cout<< "USAGE: ./dLE_pre_average_rescale [OPTIONS]" << endl;
    cout << " " <<endl;
    ...
    cout<< "The input file is a result of a high-dimensional binnning of the forward and backward displacements, v and w, and the products " << endl;
    ...
    cout << "until their summed weights (i.e., the number of points which were matched by the binning applied on the system trajectory) match or outnumber" <<endl;
    ...
    cout << " " <<endl;
    cout << "The code allows also for the application of the rescaled-friction dLE. Having estimated the fields, the matrix M (given as file via -alterfric)" <<endl;
    ...
    cout << " " << " " << "-i name input trajectory (x1,...,xn,v1,...vn,w1,...,wn,v1*v1,v1*v2,...vn*vn,w1*w1,...v1*w1,...,weight) [default input]" << endl;
    cout << " " << " " << "-o name output trajectory (x1,..,xn,f1,...,fn,g11,g12,...,K11,K21,...,xi1,...,xin,used_weights,max_distance) [default input.lang]" << endl;
...
std::vector<double> startposition;                                                      //Starting point is the input point with the higest weight
int weigthstartposition=0;
...
while(datafieldestimate>>transferposition[0]){                                          //read in input, determine value range and dLE startposition
    ...
    lengthinput++;
    if(b==0){                                                                              //determine value range of input and starting point of dLE (input point with the highest weigth)
        for(a=0;a<dim;a++){
            mininput.push_back(transferposition[a]);
            maxinput.push_back(transferposition[a]);
            startposition.push_back(transferposition[a]);
            weigthstartposition=transferweights;
        }
    }
    else{
        for(a=0;a<dim;a++){
        ...
        if(transferweights>=weigthstartposition){
            weigthstartposition=transferweights;
            for(a=0;a<dim;a++){
...
for(a=0;a<length;a++){
    ...
    diffusion=massinverse*(alterfrictionmatrix*(mass*diffusion))*rescale*sqrt(rescale);
