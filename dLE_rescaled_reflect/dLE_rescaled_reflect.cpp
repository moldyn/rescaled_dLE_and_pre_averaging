...
for (int i = 1; i < argc; ++i) {
  if(std::string(argv[i]) == "-h"){
    cout << " " <<endl;
    cout << "USAGE: ./dLE_rescaled_reflect [OPTIONS]" << endl;
    cout << " " <<endl;
    ...
    cout << " " <<endl;
    cout << "In addition, the code allows to rescale the explicit timestep dependence of the dLE fields by some factor (option -resc). This means practically" << endl;
    cout << "that the dLE is integrated with the timestep dt_int=dt_input*resc, i.e., the trajectory points are seperated by dt_int and not dt_input." << endl;
    cout << " " <<endl;
    ...
    cout << " " <<endl;
    cout << "In contrast to the normal dLE, this algorithm reflects the trajectory at some upper and lower border defined for every coordinate seperately and given to" <<endl;
    cout << "the program by some file (name specified by option -reflec). If the trajectory jumps over one of those borders by the distance a, this coordinate is set" <<endl;
    cout << "back to x=border-a. In addition, the velocity is reversed. " <<endl; 
    cout << "The file given to the code should have as many lines as -d specifies the dimensionality of the problem, the nth line gives x_min xmax" <<endl;
    cout << "as first and second column for the nth coordinate of the dLE (nth column of file -i)." <<endl;
    cout << " " <<endl;
    ...
    cout << " " << " " << "-resc rescale explicit timestep dependence of dLE fields, i.e., dLE is integrated with other dt [default: 0.1, i.e., downscale integration timestep by 10]" << endl;
    cout << " " << " " << "-alterfric name matrix file used to alter friction and noise [default: modifyfriction] " << endl;
    cout << " " << " " << "-reflec name of file containing the lower and upper borders [default: borders]" << endl;
    cout << " " <<endl;
    cout << " " <<endl;
    exit(0);
  }
...
std::string nameborders="borders";
...
std::vector<double> lowerreflectborders;                                                // contains the borders where the trajectory should be reflected
std::vector<double> upperreflectborders;
...
for(a=1;a<argc;a++){
    ...
    else if(std::string(argv[a])=="-resc"){
        if(a+1<argc){
		rescale=atof(argv[a+1]);
        }
    }
    else if(std::string(argv[a])=="-reflec"){
        if(a+1< argc){ 
		nameborders=argv[a+1];
        }
    }
}

cout << "Rescale integration timestep by" << " " << rescale << endl;
...
for(a=0;a<dim;a++){
  ...
  lowerreflectborders.push_back(0); 
  upperreflectborders.push_back(0); 
}
...
ifstream datareflect;
datareflect.open(nameborders,ios::in);

for(a=0;a<dim;a++){
    datareflect>>lowerreflectborders[a];
    datareflect>>upperreflectborders[a];
    if(upperreflectborders[a]<lowerreflectborders[a]){
        cout << "Borders odd, for dimension " << a+1 << " " << "the upper border is smaller than the lower border."<<endl;
        exit(0);
    }
}
...
cout << "Reflection borders detected" << endl;

for(a=0;a<dim;a++){
    cout << lowerreflectborders[a] << " " << upperreflectborders[a] << endl;
}
...
for(a=0;a<length;a++){
    ...
    helpmatrix=massinverse*alterfrictionmatrix*((mass*helpmatrix)*alterfrictionmatrixtranspose)*rescale;            //Alter friction matrix and rescale explicite timestep dependence
    for(c=0;c<dim;c++){ 
        for(d=0;d<dim;d++){
    ...
    diffusion=massinverse*(alterfrictionmatrix*(mass*diffusion))*rescale*sqrt(rescale);                             //Alter noise amplitude and rescale explicit timestep dependence
    
    for(c=0;c<dim;c++){                                                                                                                                          //generate noise vector
        noise(c)=randomnumber();
    }
    for(c=0;c<dim;c++){                                                                             //Rescale the explicit timestep dependence of the drift field
        drift(c)=drift(c)*rescale*rescale;
    }
    ...
    for(c=0;c<dim;c++){                                                                                               //if propagation crosses borders, this loop sets it back by a=x-border, x'=border-a
        if(next_position(c)>upperreflectborders[c]){                                                                  //upper border
            shiftaround=next_position(c)-upperreflectborders[c];
            next_position(c)=upperreflectborders[c]-shiftaround;
            position(c)=(2*next_position(c))-position(c);                                                             //In these two lines, I reflect the velocity
            position(c)=position(c)+(2*shiftaround);
        }
        if(next_position(c)<lowerreflectborders[c]){                                                                  //lower border
            shiftaround=next_position(c)-lowerreflectborders[c];
            next_position(c)=lowerreflectborders[c]-shiftaround;
            position(c)=(2*next_position(c))-position(c);                                                             //In these two lines, I reflect the velocity
            position(c)=position(c)+(2*shiftaround);
        }
    }
