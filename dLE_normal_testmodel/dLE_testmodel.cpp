...
for (int i = 1; i < argc; ++i) {
  if(std::string(argv[i]) == "-h"){
    cout << " " <<endl;
    cout << "USAGE: ./dLE_testmodel [OPTIONS]" << endl;
    cout << " " <<endl;
    ...
    cout << " " << " " << "-o name output trajectory (y1,..,yn,f1,...,fn,g11,g12,...,K11,K21,...,xi1,...,xin,future,max_distance) [default input.lang]" << endl;
...
while(datafieldestimate>>transferposition[0]){                                          //read in input, determine value range
    for(a=1;a<dim;a++){
    ...
    if(statusactualpoint!=0){                                                                                               //only consider points which have a follower
        if(statusprevpoint!=0){                                                                                             //only consider points which have a predecessor
            ...
            for(b=0;b<numberboxes;b++){                                                                                     //determine actual k nearest neighbours
                if(boxshouldbeconsidered[b]==1){                                                                            //use box assistance, i.e., the result of the binning above
                    for(c=0;c<entriesassigendtoboxes[b];c++){
                        for(d=0;d<dim;d++){
                            transferposition[d]=positionsassigntoboxes[b][c][d];
                        }
                        transferdistance=0;
                        transferbox=b;
            ...
            for(b=0;b<nrneighbours;b++){
                ...
                for(c=0;c<dim;c++){
                    neighbourhoodpositions[b][c]=positionsassigntoboxes[transferbox][transferpointnumber][c];
                    neighbourhoodbackwarddisplacements[b][c]=backwardvelocityassigendtoboxes[transferbox][transferpointnumber][c];
                    neighbourhoodforwarddisplacements[b][c]=forwardvelocitiesassignedtoboxes[transferbox][transferpointnumber][c];
                }
            ...
            b=0;
                while(sumweight<nrneighbours){                                                                                                                //calculate primary sums which lead to the averages/covariances from the neighbourhood
                    for(c=0;c<dim;c++){
                        transferposition[c]=neighbourhoodpositions[b][c];
                        transferforwarddisplacement[c]=neighbourhoodforwarddisplacements[b][c];
                        transferbackwarddisplacement[c]=neighbourhoodbackwarddisplacements[b][c];
                    }
